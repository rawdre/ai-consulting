...
const stateDir = path.join(root, ".cache", "agent-control", "inbox-reader");
...
function extractInboxQueue(data) {
  const replyNow = data?.crm?.repliesOwedNow || [];
  const slackReplyNow = data?.slack?.replyNow || [];
  const candidates = [];

  for (const entry of replyNow) {
    if (!entry?.leadId) continue;
    candidates.push({
      leadId: entry.leadId,
      leadName: entry.leadName,
      stage: entry.stage || "Replies owed now",
      reason: entry.whyNow || "Client is waiting on Andre.",
      link: entry.link || null,
      source: "close_reply_now",
    });
  }

  for (const entry of slackReplyNow) {
    if (!entry?.leadId) continue;
    if (candidates.some((item) => item.leadId === entry.leadId)) continue;
    candidates.push({
      leadId: entry.leadId,
      leadName: entry.leadName,
      stage: entry.stage || "Slack inbound",
      reason: entry.whyNow || "Slack flagged live inbound activity.",
      link: entry.link || null,
      source: "slack_reply_now",
    });
  }

  return candidates.slice(0, Number(process.env.INBOX_READER_BATCH_SIZE || "4"));
}
...
function buildSummary(result) {
  if (result.drafted_count > 0) {
    return `Prepared ${result.drafted_count} inbox draft${result.drafted_count === 1 ? "" : "s"} from ${result.queue_size} live inbox lead${result.queue_size === 1 ? "" : "s"}.`;
  }
  if (result.queue_size > 0) {
    return `Inbox queue has ${result.queue_size} live lead${result.queue_size === 1 ? "" : "s"}, but none needed a fresh draft in this pass.`;
  }
  return "No live inbox replies needed drafting in this pass.";
}
...
async function main() {
  if (!process.env.CLOSE_API_KEY) {
    throw new Error("Inbox Reader cannot run because CLOSE_API_KEY is missing.");
  }

  const data = await readJson(missionControlFile);
  const queue = extractInboxQueue(data);
  ...
  const drafted = [];
  const skipped = queue
    .filter((entry) => alreadyDrafted.has(entry.leadId))
    .map((entry) => ({

(() => {
  ...
  async function send(){
    ...
    // Keep the same ID and estimate number when retrying so the receiver can deduplicate uncertain deliveries.
    const requestId=lastPayload&&lastPayload.order_summary_text===text?lastPayload.request_id:crypto.randomUUID();
    const quote=core.buildQuoteEmail(state,e,{requestId});
    const payload={source:'Customer Catering Package Builder',request_id:requestId,submitted_at:new Date().toISOString(),subject:quote.subject,team_subject:`New estimate & callback request · ${state.name} · ${state.event} · ${state.date}`,customer_name:state.name,email:state.email,phone:state.phone,event_date:state.date,event_date_iso:state.date,event_time:state.time,guests:+state.guests,event_type:state.event,service_style:state.style,venue:state.venue||'',street_address:state.street,address_line_2:state.unit||'',city:state.city,state:state.state,zip_code:state.zip,full_address:[state.street,state.unit,state.city,state.state,state.zip].filter(Boolean).join(', '),callback_requested:true,callback_time:state.callback,contact_consent:true,notes:[state.notes,state.diet,state.access].filter(Boolean).join('\n'),dietary_needs:state.diet||'',access_notes:state.access||'',items,items_count:items.length,estimate_number:quote.estimateNo,quote_delivery_mode:quote.deliveryMode,create_note:quote.createNote,order_summary_text:text,email_fallback_text:quote.text,customer_quote_html:quote.html,team_notification_html:quote.html,notification_email:team,catering_total:e.food,staff_fee:e.staff,service_charge:e.service,ma_tax:e.tax,event_total:e.total,event_total_formatted:money(e.total),estimated_subtotal:e.subtotal,estimated_subtotal_formatted:money(e.subtotal),pricing_status:'Estimate only; delivery and travel pending',callback_action:'Call customer to finalize catering quote',wants_service:state.style==='Drop-Off'?'No':'Yes'};
    lastPayload=payload;
  ...
  $('download').addEventListener('click',()=>{const url=URL.createObjectURL(new Blob([lastPayload.customer_quote_html],{type:'text/html;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download=`${lastPayload.estimate_number}-Comeketo-estimate.html`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
